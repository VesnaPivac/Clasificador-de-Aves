...
function mostrarCamara() {
	...
	navigator.mediaDevices.getUserMedia(opciones)
		...
		.catch(function(err) {
			alert("Error: No se pudo utilizar la camara");
			console.log(err);
			alert(err);
		})
	} else {
	alert("Error: No existe la funcion getUserMedia");
	}
}
...
function predecir() {
	if (modelo != null) {
	resample_single(canvas, 300, 300, othercanvas);

	//Predicción
	var ctx2 = othercanvas.getContext("2d");
	var imgData = ctx2.getImageData(0, 0, 300, 300);
	ctx2.canvas.willReadFrequently = true;
	...
	var maxProbIndex = resultado.indexOf(Math.max.apply(null, resultado));
	...
	'ZEBRA DOVE'];
	console.log("Prediccion", clases[maxProbIndex]);
    document.getElementById("resultado").innerHTML = clases[maxProbIndex];
